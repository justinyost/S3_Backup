...
then
	...
	if $MYSQL_DUMP_ACTIVE;
	then
		mysqldump --add-drop-table --all-databases --user=${MYSQL_DB_USERNAME} --password=$MYSQL_DB_PASSWORD | gzip -9 > ${BACKUP_LOCAL_PATH}weekly_db_backup_mysql_${THEDATE}.sql.gz
	fi
	if $POSTGRESQL_DUMP_ACTIVE;
	then
		pg_dumpall --username=${POSTGRES_DB_USERNAME} | gzip -9 > ${BACKUP_LOCAL_PATH}weekly_db_backup_postgresql_${THEDATE}.sql.gz
	fi
	if $MONGODB_DUMP_ACTIVE;
	then
		mongodump --host localhost --username ${MONGO_DB_USERNAME} --password ${MONGO_DB_PASSWORD} | gzip -9 > ${BACKUP_LOCAL_PATH}weekly_db_backup_mongodb_${THEDATE}.sql.gz
	fi
...
then
	...
	if $MYSQL_DUMP_ACTIVE;
	then
		mysqldump --add-drop-table --all-databases --user=${MYSQL_DB_USERNAME} --password=$MYSQL_DB_PASSWORD | gzip -9 > ${BACKUP_LOCAL_PATH}monthly_db_backup_mysql_${THEDATE}.sql.gz
	fi
	if [${POSTGRESQL_DUMP_ACTIVE:=true} == TRUE]
	then
		pg_dumpall --username=${POSTGRES_DB_USERNAME} | gzip -9 > ${BACKUP_LOCAL_PATH}monthly_db_backup_postgresql_${THEDATE}.sql.gz
	fi
	if $MONGODB_DUMP_ACTIVE;
	then
		mongodump --host localhost --username ${MONGO_DB_USERNAME} --password ${MONGO_DB_PASSWORD} | gzip -9 > ${BACKUP_LOCAL_PATH}monthly_db_backup_mongodb_${THEDATE}.sql.gz
	fi
